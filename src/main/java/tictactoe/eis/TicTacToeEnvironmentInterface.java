...
public class TicTacToeEnvironmentInterface extends EIDefaultImpl {
	...
	private enum PlayerReasoner {
		 ...
		 *                             {@link PlayerReasoner}.
		 */
		public static PlayerReasoner fromString(String engine) throws ManagementException {
			try {
	...
	public void actionoccupy(int position, String player) {
		...
		if (currentConfiguration.getCurrentPlayerType() == playerType) {
			...
			if (TicTacToe.getConfiguration().getPlayerAtSquare(position - 1) == PlayerType.NONE) {
				...
				Percept percept = new Percept("pos", new Numeral(position), new Identifier(playerType.toString()));
				try {
			...
			if (!currentConfiguration.isEndOfGame()) {
				if (currentConfiguration.getCurrentPlayer().getKindofPlayer() == PlayerKind.AI) {
					int p = ((PlayerAI) currentConfiguration.getCurrentPlayer()).coupOrdi();
	 ...
	 */
	@Override
	protected List<Percept> getAllPerceptsFromEntity(String entity) throws PerceiveException, NoEnvironmentException {
		List<Percept> percepts = new LinkedList<>();

		PlayerType currentPlayer = TicTacToe.getConfiguration().getCurrentPlayerType();
		if (currentPlayer != PlayerType.NONE) {
			percepts.add(new Percept("turn", new Identifier(currentPlayer.toString())));
		}

		PlayerType[][] gameConfiguration = TicTacToe.getConfiguration().getGameBoardConfiguration();
		...
		if (winner != null) {
			if (winner != PlayerType.NONE) {
				percepts.add(new Percept("winner", new Identifier(winner.toString())));
			} else {
			...
			try {
				...
				System.out.print("End of Tic-Tac-Toe game but could not change EIS to PAUSED state.");
			}
		}
	...
	protected Percept performEntityAction(String entity, Action action) throws ActException {
		...
		int position = ((Numeral) params.get(0)).getValue().intValue();
		...
		return null;
		// TODO: new Percept("pos", new Numeral(position), new Identifier());
	}
	 ...
	 */
	private void processParameters(Map<String, Parameter> parameters) throws ManagementException {
